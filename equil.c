...
double   sum_rho=0.0,         /* intermediate sum in eqn for rho */
  ...
  while(((dif> accuracy)||(dif2> accuracy)) || n_of_it<2) { 
      ...
      */        
      ...
      r_eq=sqrt(SQ(r_e_old)*2.0*( h_center)/((gama_pole_h+rho_pole_h-gama_center_h-rho_center_h)*SQ(r_out)));
      ...
      r_eDM_old=r_eDM;      
      s_pDM_new=r_ratioDM*r_eDM/(r_ratioDM*r_eDM+r_out);            
      ...
      r_eDM=sqrt(SQ(r_eDM_old)*2.0*( h_centerDM)/((gama_pole_hDM+rho_pole_hDM-gama_center_h-rho_center_h)*SQ(r_out)));
...
//printf("%g %g %g %g %g\n",velocity_sq[1][1],alpha[1][1], gama[1][1], rho[1][1], omega[1][1]);
    ...
    (*OmegaDM) = Omega_hDM*C/(r_out*sqrt(KAPPA));
