 ...
 int size=RDIV-Num;
  ...
  double sum=0.0,          /* final sum */
    ...
    if(size%2==0){
      ...
      for(int i=Num;i<=Num+(RDIV-Num)/2-1;i+=1) {
      if((p1[2*i]!=0+p2[2*i])!=0){
      dx=(p2[2*i]+p1[2*i]-p2[2*i+1]-p1[2*i+1]);
      dx1=(p2[2*i+1]+p1[2*i+1]-p2[2*i+2]-p1[2*i+2]);
       sum+= (dx+dx1)/6.*((2-dx1/dx)*f(p2[2*i]+p1[2*i],e2[2*i]+e1[2*i])+(dx+dx1)*(dx+dx1)*f(p2[2*i+1]+p1[2*i+1],e2[2*i+1]+e1[2*i+1])/(dx*dx1)+(2-dx/dx1)*f(p2[2*i+2]+p1[2*i+2],e2[2*i+2]+e1[2*i+2]));
              
       }
      }
    }else{
      for(int i=Num;i<=Num+(RDIV-1-Num)/2;i+=1) {
      if((p1[2*i]!=0+p2[2*i])!=0){
      dx=(p2[2*i]+p1[2*i]-p2[2*i+1]-p1[2*i+1]);
      dx1=(p2[2*i+1]+p1[2*i+1]-p2[2*i+2]-p1[2*i+2]);
       sum+= (dx+dx1)/6.*((2-dx1/dx)*f(p2[2*i]+p1[2*i],e2[2*i]+e1[2*i])+(dx+dx1)*(dx+dx1)*f(p2[2*i+1]+p1[2*i+1],e2[2*i+1]+e1[2*i+1])/(dx*dx1)+(2-dx/dx1)*f(p2[2*i+2]+p1[2*i+2],e2[2*i+2]+e1[2*i+2]));
      ...
      dx=(p2[RDIV-1]+p1[RDIV-1]-p2[RDIV]-p1[RDIV]);
      dx1=(p2[RDIV-2]+p1[RDIV-2]-p2[RDIV-1]-p1[RDIV-1]);

     sum+=  (2*dx*dx+3*dx*dx1)/(6*(dx1+dx2))*f(p2[1]+p1[1],e2[1]+e1[1])+(dx*dx+3*dx*dx1)/(6*(dx2))*f(p2[2]+p1[2],e2[2]+e1[2])+(dx*dx*dx)/(6*dx2*(dx1+dx))*f(p2[3]+p1[3],e2[3]+e1[3]);
    }
  ...
  /*
  for(int i=Num;i<=RDIV-2;i+=2){
     dx=p2[i]+p1[i]-p2[i+1]-p1[i+1];
     dx1=p2[i+1]+p1[i+1]-p2[i+2]-p1[i+2];
     //printf("%lf %lf \n",sum,( f( p1[i]+p2[i],e1[i]+e2[i])  )*(dx/3.0));
     sum =sum+ (f( p1[i]+p2[i],e1[i]+e2[i])*dx  +4*f(p1[i+1]+p2[i+1],e1[i+1]+e2[i+1])*(dx+dx1)/2+f(p1[i+2]+p2[i+2],e1[i+2]+e2[i+2])*dx1  )/3.0;
   }*/
  return (sum);
}
...
double y_at_x(double x,double x1, double x2, double y1, double y2){

  if(y1>=0.0){

  double a=(log10(y1)-log10(y2))/(log10(x1)-log10(x2));
  double b=pow(10,log10(y2)-(log10(y1)-log10(y2))/(log10(x1)-log10(x2))*log10(x2));
  return b*pow(x,a);
  }else{
  y1=-y1;
  y2=-y2;
...
double dpds(double p, double p_surface, double e,double rho_s,double gamma_s,double s,double v,double Omega,double omega,double omega_s){
      if(p<p_surface){
        return 0;
      }else{
        if(v==0){
   	   return -(e+p)*(rho_s*0.5+gamma_s*0.5);
        }else{
	   return -(e+p)*(rho_s*0.5+gamma_s*0.5+v/(1.0-v)*(rho_s+omega_s/(Omega-omega)-1/(s*(1-s))));
        }
      }
}

double dpdm(double p, double p_surface, double e,double rho_m,double gamma_m,double m,double v,double Omega,double omega,double omega_m){
      if(p<p_surface){
        return 0;
      }else{
        if(v==0){
   	   return -(e+p)*(rho_m*0.5+gamma_m*0.5);
        }else{
	   return -(e+p)*(rho_m*0.5+gamma_m*0.5+v/(1.0-v)*(rho_m+omega_m/(Omega-omega)+1/(1-m*m)));
        }
      }	  
}
...
/* Circumferential radius */
 ...
 (*R_e) = sqrt(KAPPA)*r_e*exp((interp(s_gp,gama_mu_0,SDIV,(r_e/r_eDM_old)/(1.+(r_e/r_eDM_old)), &n_nearest) -interp(s_gp,rho_mu_0,SDIV,(r_e/r_eDM_old)/(1.+(r_e/r_eDM_old)), &n_nearest))/2.0); 
 
  double R_p=sqrt(KAPPA)*r_ratio*r_e*exp((interp(s_gp,gama_mu_1,SDIV,(r_ratio*r_e/r_eDM_old)/(1.+(r_ratio*r_e/r_eDM_old)), &n_nearest)-interp(s_gp,rho_mu_1,SDIV,(r_ratio*r_e/r_eDM_old)/(1.+(r_ratio*r_e/r_eDM_old)), &n_nearest))/2.0);
 (*Ratio_sch)=R_p/(*R_e);
 }
   ...
   index = 1;
    ...
    if(r_ratio==1.0) 
         J=0.0; 
          ...
          if((strcmp(eos_type,"tab")==0) || (strcmp(eos_type,"DM")==0)) 
              J *= 4*PI*KAPPA*C*C*C*pow(r_eq,4.0)/G;
    }
...

{
 ...
 for(s=1;s<=SDIV;s++) {
    ...
    if(r_is_s<R_is) {
      ...
      nu_s=log((1.0-(m_final+mDM_final)/(2.0*r_is_s))/(1.0+(m_final+mDM_final)/(2*r_is_s)));
    }
...
double   sum_rho=0.0,         /* intermediate sum in eqn for rho */
    ...
    n_nearest = n_tab/2;
    n_nearestDM = n_tabDM/2;
      r_eq = max((*r_e_new),(*rDM_e_new));
      r_eDM=(((*r_e_new)>(*rDM_e_new))?(*rDM_e_new):(*r_e_new));

      double n0_center,n0_centerDM,e_center,e_centerDM,p_center,p_centerDM,rho_center,rho_centerDM;
      p_center=p_at_h(h_center,pow(10,log_h_tab[1]), log_p_tab,log_h_tab, n_tab, &n_nearest);
      e_center=e_at_p(p_center,pow(10,log_p_tab[1]), log_e_tab, log_p_tab, n_tab, &n_nearest, eos_type,Gamma_P);
      n0_center=n0_at_e(e_center,pow(10,log_e_tab[1]), log_n0_tab, log_e_tab, n_tab,&n_nearest);
      rho_center=(e_center+p_center)*exp(-h_center);

      p_centerDM=p_at_h(h_centerDM,pow(10,log_h_tabDM[1]), log_p_tabDM,log_h_tabDM, n_tabDM, &n_nearestDM);
      e_centerDM=e_at_p(p_centerDM,pow(10,log_p_tabDM[1]), log_e_tabDM, log_p_tabDM, n_tabDM, &n_nearestDM, eos_typeDM,Gamma_PDM);
      n0_centerDM=n0_at_e(e_centerDM,pow(10,log_e_tabDM[1]), log_n0_tabDM, log_e_tabDM, n_tabDM,&n_nearestDM);
      rho_centerDM=(e_centerDM+p_centerDM)*exp(-h_centerDM);
  ...
  while(((dif> accuracy)||(dif2> accuracy)) || n_of_it<2) { 
    ...
    n_nearestDM = n_tabDM/2;
      ...
      //r_eq=sqrt(2*( log((exp(h_center)*rho_center +exp(h_centerDM)*rho_centerDM)/(rho_center+rho_centerDM)) )/(gama_pole_h+rho_pole_h-gama_center_h-rho_center_h));
       if(((*r_e_new)>=(*rDM_e_new))){
       r_eq=sqrt(2*( h_center-enthalpy_min)/(gama_pole_h+rho_pole_h-gama_center_h-rho_center_h));

       r_eDM=sqrt(SQ(r_eDM_old)*2*( h_centerDM-enthalpy_minDM)/((gama_pole_hDM+rho_pole_hDM-gama_center_h-rho_center_h)*SQ(r_e_old)));
       ...
       r_eq=sqrt(2*( h_centerDM-enthalpy_minDM)/(gama_pole_h+rho_pole_h-gama_center_h-rho_center_h));

       r_eDM=sqrt(SQ(r_eDM_old)*2*( h_center-enthalpy_min)/((gama_pole_hDM+rho_pole_hDM-gama_center_h-rho_center_h)*SQ(r_e_old)));       
      ...
      if(r_ratio==1.0) {
       ...
       if(r_ratioDM==1.0) {
        ...
        omega_equator_hDM=0.0;        
            ...
            omega_equator_hDM=interp(s_gp,omega_mu_0,SDIV,s_eDM_new, &n_nearest)*r_eDM_old/r_e_old;
            term_in_Omega_hDM=1.0-exp(SQ(r_eDM*r_e_old/r_eDM_old)*(gama_pole_hDM+rho_pole_hDM-gama_equator_hDM-rho_equator_hDM));
            if(term_in_Omega_hDM>=0.0) {
               Omega_hDM = omega_equator_hDM + exp(SQ(r_eDM*r_e_old/r_eDM_old)*rho_equator_hDM) *sqrt(term_in_Omega_hDM);
      ...
      for(s=1;s<=SDIV;s++) {
        ...
        if((*r_e_new)>(*rDM_e_new)){
            ...
            if((enthalpy[s][m]<=enthalpy_min) || (sgp>s_e)) {
                  ...
                  enthalpy[s][m]=0.0;
                     ...
                     pressure[s][m]=p_at_h(enthalpy[s][m],pow(10,log_h_tab[1]), log_p_tab, 
                                           log_h_tab, n_tab, &n_nearest);
                     energy[s][m]=e_at_p(pressure[s][m],pow(10,log_p_tab[1]), log_e_tab, 
       ...
       //printf("%f %f %f %f %f %f %f %f \n",r_eq*s_gp[s]/(1-s_gp[s]),dif,dif2,r_eDM,rho_pole_hDM,gama_center_h,rho_center_h);
	    ...
	    enthalpyDM[s][m]=enthalpy_minDM + 0.5*(SQ(r_e_old*r_eDM/r_eDM_old)*(gama_pole_hDM+rho_pole_hDM
                           -gama[s][m]-rsm)-log(1.0-velocity_sq[s][m]));
            ...
            if((enthalpyDM[s][m]<=enthalpy_minDM) || (sgp>s_eDM_new)) {
                  ...
                  enthalpyDM[s][m]=0.0;
                     ...
                     pressureDM[s][m]=p_at_h(enthalpyDM[s][m],pow(10,log_h_tabDM[1]), log_p_tabDM, 
                                           log_h_tabDM, n_tabDM, &n_nearestDM);
                     energyDM[s][m]=e_at_p(pressureDM[s][m],pow(10,log_p_tabDM[1]), log_e_tabDM, 
                                      log_p_tabDM, n_tabDM, &n_nearestDM, eos_typeDM,
                                       Gamma_PDM);
	           }
           }else{
           
                 enthalpy[s][m]=enthalpy_min + 0.5*(SQ(r_e_old*r_eDM/r_eDM_old)*(gama_pole_hDM+rho_pole_hDM
                           -gama[s][m]-rsm)-log(1.0-velocity_sq[s][m]));
	...
	if(s==1){
	    pressure[s][m]=p_center;
            energy[s][m]=e_center;
            enthalpy[s][m]=h_center;
	    pressureDM[s][m]=p_centerDM;
            energyDM[s][m]=e_centerDM;             
            enthalpyDM[s][m]=h_centerDM;
	}else{
	 if((m==1)){
          d_gama_s=deriv_s(gama,s-1,m);
          d_gama_m=deriv_m(gama,s-1,m);
          d_rho_s=deriv_s(rho,s-1,m);
          d_rho_m=deriv_m(rho,s-1,m);
          d_omega_s=deriv_s(omega,s-1,m);
          d_omega_m=deriv_m(omega,s-1,m);
          
          dum_e=e_at_p(pressure[s-1][m],pow(10,log_p_tab[1]), log_e_tab, log_p_tab, n_tab, &n_nearest, eos_type, Gamma_P);
          k0= dpds( pressure[s-1][m],pow(10,log_p_tab[1]), dum_e,d_rho_s,d_gama_s,s_gp[s],velocity_sq[s-1][m],Omega_h,omega[s-1][m],d_omega_s);
         
  	 dum_e=e_at_p(pressure[s-1][m]+DS*0.5*k0,pow(10,log_p_tab[1]), log_e_tab, log_p_tab, n_tab, &n_nearest, eos_type, Gamma_P); 
          k1= dpds( pressure[s-1][m]+DS*0.5*k0,pow(10,log_p_tab[1]), dum_e,d_rho_s,d_gama_s,s_gp[s]+DS/2.,velocity_sq[s-1][m],Omega_h,omega[s-1][m],d_omega_s);

  	  dum_e=e_at_p(pressure[s-1][m]+DS*0.5*k1,pow(10,log_p_tab[1]), log_e_tab, log_p_tab, n_tab, &n_nearest, eos_type, Gamma_P); 
          k2= dpds( pressure[s-1][m]+DS*0.5*k1,pow(10,log_p_tab[1]), dum_e,d_rho_s,d_gama_s,s_gp[s]+DS/2.,velocity_sq[s-1][m],Omega_h,omega[s-1][m],d_omega_s);

  	  dum_e=e_at_p(pressure[s-1][m]+DS*k2,pow(10,log_p_tab[1]), log_e_tab, log_p_tab, n_tab, &n_nearest, eos_type, Gamma_P); 
          k3= dpds( pressure[s-1][m]+DS*k2,pow(10,log_p_tab[1]), dum_e,d_rho_s,d_gama_s,s_gp[s]+DS,velocity_sq[s-1][m],Omega_h,omega[s-1][m],d_omega_s);
          
	pressure[s][m]=pressure[s-1][m]+(k0+2.*k1+2.*k2+k3)*DS/6.;	
	if((pressure[s][m]<pow(10,log_p_tab[1]))|| (sgp>s_e)){ 
	    pressure[s][m]=0;
	}else{
	  if((m==1)){
	    s_e_new=s_gp[s+1];
	    //r_eq=r_e_old*s_e_new/(1-s_e_new);
	  }
	}
	
        energy[s][m]=e_at_p(pressure[s][m],pow(10,log_p_tab[1]), log_e_tab,log_p_tab, n_tab, &n_nearest, eos_type,Gamma_P); 
        enthalpy[s][m]=h_at_p(pressure[s][m],pow(10,log_p_tab[1]), log_h_tab, log_p_tab, n_tab, &n_nearest);



          dum_e=e_at_p(pressureDM[s-1][m],pow(10,log_p_tabDM[1]), log_e_tabDM, log_p_tabDM, n_tabDM, &n_nearestDM, eos_typeDM, Gamma_PDM);
          kDM0= dpds( pressureDM[s-1][m],pow(10,log_p_tabDM[1]), dum_e,d_rho_s,d_gama_s,s_gp[s],velocity_sq[s-1][m],Omega_h,omega[s-1][m],d_omega_s);

 	 dum_e=e_at_p(pressureDM[s-1][m]+DS*0.5*kDM0,pow(10,log_p_tabDM[1]), log_e_tabDM, log_p_tabDM, n_tabDM, &n_nearestDM, eos_typeDM, Gamma_PDM); 
          kDM1= dpds( pressureDM[s-1][m]+DS*0.5*kDM0,pow(10,log_p_tabDM[1]), dum_e,d_rho_s,d_gama_s,s_gp[s]+DS/2.,velocity_sq[s-1][m],Omega_h,omega[s-1][m],d_omega_s);

  	  dum_e=e_at_p(pressureDM[s-1][m]+DS*0.5*kDM1,pow(10,log_p_tabDM[1]), log_e_tabDM, log_p_tabDM, n_tabDM, &n_nearestDM, eos_typeDM, Gamma_PDM); 
          kDM2= dpds( pressureDM[s-1][m]+DS*0.5*kDM1,pow(10,log_p_tabDM[1]), dum_e,d_rho_s,d_gama_s,s_gp[s]+DS/2.,velocity_sq[s-1][m],Omega_h,omega[s-1][m],d_omega_s);

  	  dum_e=e_at_p(pressureDM[s-1][m]+DS*kDM2,pow(10,log_p_tabDM[1]), log_e_tabDM, log_p_tabDM, n_tabDM, &n_nearestDM, eos_typeDM, Gamma_PDM); 
          kDM3= dpds( pressureDM[s-1][m]+DS*kDM2,pow(10,log_p_tabDM[1]), dum_e,d_rho_s,d_gama_s,s_gp[s]+DS,velocity_sq[s-1][m],Omega_h,omega[s-1][m],d_omega_s);
          
	pressureDM[s][m]=pressureDM[s-1][m]+(kDM0+2.*kDM1+2.*kDM2+kDM3)*DS/6.;
        if((pressureDM[s][m]<pow(10,log_p_tabDM[1]))|| (sgp>s_e)){ 
	    pressureDM[s][m]=0;
	}
	
	
        energyDM[s][m]=e_at_p(pressureDM[s][m],pow(10,log_p_tabDM[1]), log_e_tabDM,log_p_tabDM, n_tabDM, &n_nearestDM, eos_typeDM, Gamma_PDM); 
        enthalpyDM[s][m]=h_at_p(pressureDM[s][m],pow(10,log_p_tabDM[1]), log_h_tabDM, log_p_tabDM, n_tabDM, &n_nearestDM);      




       }else{
          d_gama_s=deriv_s(gama,s,m-1);
          d_gama_m=deriv_m(gama,s,m-1);
          d_rho_s=deriv_s(rho,s,m-1);
          d_rho_m=deriv_m(rho,s,m-1);
          d_omega_s=deriv_s(omega,s,m-1);
          d_omega_m=deriv_m(omega,s,m-1);
          
          dum_e=e_at_p(pressure[s][m-1],pow(10,log_p_tab[1]), log_e_tab, log_p_tab, n_tab, &n_nearest, eos_type, Gamma_P);
          l0= dpdm( pressure[s][m-1],pow(10,log_p_tab[1]), dum_e,d_rho_m,d_gama_m,mu[m],velocity_sq[s][m-1],Omega_h,omega[s][m-1],d_omega_m);
         
  	 dum_e=e_at_p(pressure[s][m-1]+DM*0.5*l0,pow(10,log_p_tab[1]), log_e_tab, log_p_tab, n_tab, &n_nearest, eos_type, Gamma_P); 
          l1= dpdm( pressure[s][m-1]+DM*0.5*l0,pow(10,log_p_tab[1]), dum_e,d_rho_m,d_gama_m,mu[m]+DM/2.,velocity_sq[s][m-1],Omega_h,omega[s][m-1],d_omega_m);

  	  dum_e=e_at_p(pressure[s][m-1]+DM*0.5*l1,pow(10,log_p_tab[1]), log_e_tab, log_p_tab, n_tab, &n_nearest, eos_type, Gamma_P); 
          l2= dpdm( pressure[s][m-1]+DM*0.5*l1,pow(10,log_p_tab[1]), dum_e,d_rho_m,d_gama_m,mu[m]+DM/2.,velocity_sq[s][m-1],Omega_h,omega[s][m-1],d_omega_m);

  	  dum_e=e_at_p(pressure[s][m-1]+DM*l2,pow(10,log_p_tab[1]), log_e_tab, log_p_tab, n_tab, &n_nearest, eos_type, Gamma_P); 
          l3= dpdm( pressure[s][m-1]+DM*l2,pow(10,log_p_tab[1]), dum_e,d_rho_m,d_gama_m,mu[m]+DM,velocity_sq[s][m-1],Omega_h,omega[s][m-1],d_omega_m);
          
          
	pressure[s][m]=pressure[s][m-1]+(l0)*DM;	
	if((pressure[s][m]<pow(10,log_p_tab[1]))|| (sgp>s_e)){ 
	    pressure[s][m]=0;
	}
        ...
        enthalpy[s][m]=h_at_p(pressure[s][m],pow(10,log_p_tab[1]), log_h_tab, log_p_tab, n_tab, &n_nearest);
          ...
          dum_e=e_at_p(pressureDM[s][m-1],pow(10,log_p_tabDM[1]), log_e_tabDM, log_p_tabDM, n_tabDM, &n_nearestDM, eos_typeDM, Gamma_PDM);
          lDM0= dpdm( pressureDM[s][m-1],pow(10,log_p_tabDM[1]), dum_e,d_rho_m,d_gama_m,mu[m],velocity_sq[s][m-1],Omega_h,omega[s][m-1],d_omega_m);

 	 dum_e=e_at_p(pressureDM[s][m-1]+DM*0.5*lDM0,pow(10,log_p_tabDM[1]), log_e_tabDM, log_p_tabDM, n_tabDM, &n_nearestDM, eos_typeDM, Gamma_PDM); 
          lDM1= dpdm( pressureDM[s][m-1]+DM*0.5*lDM0,pow(10,log_p_tabDM[1]), dum_e,d_rho_m,d_gama_m,mu[m]+DM/2.,velocity_sq[s][m-1],Omega_h,omega[s][m-1],d_omega_m);

  	  dum_e=e_at_p(pressureDM[s][m-1]+DM*0.5*lDM1,pow(10,log_p_tabDM[1]), log_e_tabDM, log_p_tabDM, n_tabDM, &n_nearestDM, eos_typeDM, Gamma_PDM); 
          lDM2= dpdm( pressureDM[s][m-1]+DM*0.5*lDM1,pow(10,log_p_tabDM[1]), dum_e,d_rho_m,d_gama_m,mu[m]+DM/2.,velocity_sq[s][m-1],Omega_h,omega[s][m-1],d_omega_m);

  	  dum_e=e_at_p(pressureDM[s][m-1]+DM*lDM2,pow(10,log_p_tabDM[1]), log_e_tabDM, log_p_tabDM, n_tabDM, &n_nearestDM, eos_typeDM, Gamma_PDM); 
          lDM3= dpdm( pressureDM[s][m-1]+DM*lDM2,pow(10,log_p_tabDM[1]), dum_e,d_rho_m,d_gama_m,mu[m]+DM,velocity_sq[s][m-1],Omega_h,omega[s][m-1],d_omega_m);
          

	pressureDM[s][m]=pressureDM[s][m-1]+(lDM0)*DM;
        if((pressureDM[s][m]<pow(10,log_p_tabDM[1]))|| (sgp>s_e)){ 
	    pressureDM[s][m]=0;
        }else{
	  if((m==MDIV)){
             s_eDM_new=s_gp[s+1];
             r_eDM=r_e_old*s_eDM_new/(1-s_eDM_new);
	  }
	}
        ...
        enthalpyDM[s][m]=h_at_p(pressureDM[s][m],pow(10,log_p_tabDM[1]), log_h_tabDM, log_p_tabDM, n_tabDM, &n_nearestDM);      
            ...
            /* Rescale back metric potentials (except omega) */
              ...
              // printf("%lf %lf %lf %lf %lf %lf \n",log10(fabs(r_e_old-r_eq)/r_eq),log10(dif2),r_eq,r_eDM,r_e_old, r_eDM_old);    
         /* for(s=1;s<=SDIV;s++) {
         for(m=1;m<=MDIV;m++) {
            rho[s][m] *= SQ(r_eq/r_e_old);
            gama[s][m] *= SQ(r_eq/r_e_old); 
            alpha[s][m] *= SQ(r_eq/r_e_old);
         }
      }*/
     ...
     // r_eq=sqrt(2*( h_center-enthalpy_min)/(gama_pole_h+rho_pole_h-gama_center_h-rho_center_h));
      ...
      for(s=1;s<=SDIV;s++)
         for(m=1;m<=MDIV;m++) {
            ...
            S_rho[s][m] = e_gsm*(0.5*ea*(esm + psm+(esmDM + psmDM)/*eaDM*e_gsmDM/(ea*e_gsm)*/)*s2*(1.0+v2sm)/(1.0-v2sm)
  
                          + s2*m1*SQ(e_rsm)*(SQ(s1*d_omega_s) 
                       
                          + m1*SQ(d_omega_m))
                         
                          + s1*d_gama_s - mum*d_gama_m + 0.5*rsm*(ea*(psm+psmDM/*SQ(r_e_old*r_eDM/(r_eDM_old*r_eq))*eaDM*e_gsmDM/(ea*e_gsm)*/)*s2  
 
                          - s1*d_gama_s*(0.5*s1*d_gama_s+1.0) 
 
                          - d_gama_m*(0.5*m1*d_gama_m-mum)));

            S_gama[s][m] = e_gsm*(ea*(psm+psmDM/*eaDM*e_gsmDM/(ea*e_gsm)*/)*s2 + 0.5*gsm*(ea*(psm+psmDM/*eaDM*e_gsmDM/(ea*e_gsm))*SQ(r_e_old*r_eDM/(r_eDM_old*r_eq)*/)*s2 - 0.5*SQ(s1

                           *d_gama_s) - 0.5*m1*SQ(d_gama_m)));

            S_omega[s][m]=e_gsm*e_rsm*( -ea*(Omega_h-omsm)*(esm+psm+(esmDM+psmDM)/*e_rsmDM*eaDM*e_gsmDM/(ea*e_gsm*e_rsm)*/)

                          *s2/(1.0-v2sm) + omsm*( -0.5*ea*(((1.0+v2sm)*(esm+esmDM) 
                           
                          + 2.0*v2sm*(psm+psmDM))/(1.0-v2sm))*s2 
...
//printf("%g %g %g %g\n",alpha[1][1], gama[1][1], rho[1][1], omega[1][1]);
     ...
     (*OmegaDM) = Omega_hDM*C/(r_eDM*sqrt(KAPPA));
    /* UPDATE r_e_new */
    if((*r_e_new)>(*rDM_e_new)){
      (*r_e_new) = r_eq;
      (*rDM_e_new) = r_eDM;
